...
3. **mirugit annotation file が渡された場合のみ**、それを Read して `annotations[]` を解析する。`author == "user"` のコメントが対象。`replyTo` がある場合は親 (`author: "claude"` 側) を `annotations[]` から引いて文脈として読むが、シグナルはユーザ返信側に置く。素の承認 (`LGTM` / `実行して` 等) は採用しない。採否は §2.1「mirugit annotation 上のユーザコメント」に従う。
4. §2.1 の全シグナル種別を、**対象スキル一覧に含まれるスキルに関してのみ**走査する。それ以外のスキルの話は対象外として落とす。候補は §2.1 冒頭の因果基準と末尾の突き合わせ・同根除外で絞り込む。
5. 各候補の「現象」「改善方針」に、**返す前に** §3 のサニタイズを適用する。
6. §2.2 の固定フォーマットで、サニタイズ済みの候補リストと件数だけを返す。生の会話抜粋・サニタイズ前テキスト・秘密らしき文字列は返さない。
7. 「解析はできたが拾うべき会話が無かった」（中断セッション等）はエラーではなく **0 件**。§2.2 の形で件数 0 を返す。
