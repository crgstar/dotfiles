...
| セクション | 必須 | いつ入れるか | 中身 |
|----------|----|------------|------|
| 変更内容 / Summary | ◯ | 常に | 何を変えたかを箇条書きで。コメントや整形だけの変更は、読み手の確認対象を広げるだけなので挙げない |
| 背景 / Context | △ | "なぜ" の追加情報があれば | 解決したい問題、トリガーになった事象 |
| 方針 / Approach | △ | 設計判断やトレードオフがある時 | 採った方針と、あえて入れなかった防御とその根拠。読み手が同じ検討を繰り返すときだけ、捨てた代替案も書く |
| 動作確認 / Test plan | ◯ | 常に | 検証可能なもののみ (Phase 4 参照) |

調査過程のショートハンドや内部用語はレビュアーが読める言葉に書き直す。

書くのは変更の現在の姿だけで、そこに至る作業順は書かない。「当初こう書いたが指摘を受けて直した」「一度落ちたので再実行した」は、読み手の判断を何も変えない。

```markdown
✗ 当初 A で書いたが、レビューの指摘で B に変えた
✓ B にした
✓ B にした。A は同じ問題を踏むので採れない（読み手が A を提案してくる見込みがあるときだけ）
```

参照先の issue / PR / コメントを読めば分かることは書かず、リンクだけ置く。写すと二重管理になり、参照先が更新されたときに本文だけ古くなる。本文に残すのは、この PR でしか判断していないこと。

置換の前後を見せるときは diff のコードブロックで `-` / `+` を使う。「旧:」「新:」のラベルが要らなくなり、差分として色が付く。
...
「方針」候補は思いつく限り網羅的に列挙してユーザに提示する。本文に残すのは承認を得た案で、捨てた案はカタログ表の条件に当たるときだけ書く。
