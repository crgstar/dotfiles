...
diff や文脈から継続作業とわかるマージ済み PR がある場合は、そのレビューコメントも確認し、再発しやすい指摘観点を本文作成に反映する。
...
リポジトリ外の資料を根拠に挙げるときは、資料名・URL・参照した節まで書く。同じ主題の資料が複数あると、通称だけでは読み手が別の資料を開く。

本文に「別途対応する」と書くなら issue 番号を添える。番号の無い TODO は引き取り手がないまま流れる。

既存コードを調べて数えた件数・行数・比率は本文に出さず、そこから導いた結論だけ書く。コードが変わるたび数字が古くなり、本文が追随を要求する負債になるため。読み手の判断を変える数字だけ、何をどう数えたかと併せて残す:

```markdown
✗ 呼び出し元 200 箇所のうち 197 箇所が旧 API だったので、移行は段階的にした
✓ 呼び出し元はほぼすべて旧 API なので、移行は段階的にした
```
...
必要に応じて関連リンク (Issue, 参考文献, 関連 PR) を本文中に差し込むこと。同リポの PR・Issue は `#N` で参照する。

## Phase 4: Test plan の検証可能性チェック (最重要)

各項目は「今検証できる手段があり (コマンド or 観測済み事実)、この PR の変更だけで完結し、未来事象でない」ことをすべて満たすこと。外れた項目は削除する。
...
提示前にレビュアーの目で全セクションを見直す。到達不能な参照や裏取りのない断定が残っていると差し戻される。
