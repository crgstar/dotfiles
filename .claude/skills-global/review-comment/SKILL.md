...
3. **ドラフトを見せて確認を取る**: 本文は「本文の分量」に従い、ユーザーに見せる前に同節のレビューを挟む。PRコメントは他人に見える公開行為なので、投稿先(ファイル:行、対象コミット)と本文を提示し、ユーザーの了承が出るまで `gh api` は叩かない。
...
6. **置き換えを頼む指摘は suggestion にする**: 下記同名の節に従う。
7. **投稿方式は PENDING が既定**: ユーザーが即時公開を明示していない限り、下記「PENDING(下書き)で投稿(既定)」の手順を使う。投稿前に対象PRで自分(gh認証ユーザー)のPENDINGレビューが既に開いていないか確認しておくと、後段の分岐がスムーズ。

## 本文の分量

残すのは **読み手が自力ではすぐ辿り着けない事実だけ**。調べれば分かること・既に知っていること・考えれば分かることは書かない。長い本文は読み流され、直してほしいことが埋もれるため。
...
短縮とドラフトのレビューは別コンテキスト(サブエージェント)に任せ、この節の基準と段落ごとの判定例(「この段落は要る」「これは調べれば分かる」)を渡す。引用(ファイル:行、対象コミット)と現物の照合も同じエージェントにやらせる。読み手にとって既知の事実が混ざっていることも、関心事が同居していることも、引用のずれも、自分が書いた文では見えないため。
...
現在と置換後を本文に並べる代わりに、コードフェンスの言語指定に `suggestion` を書いたブロックで出す。著者が 1 クリックで適用できるため。
