...
**コメントは現在のソースに対して書く。** 別のコメントの suggestion を適用した後の状態を前提にしたコメントは書かない。単独で適用すると壊れ、適用順にも依存するため。ファイルをまたぐ置換は 1 つの suggestion に入らないが、2 つ目のコメントを作らず、1 つ目の本文の末尾に「あわせてどこをどう直すか」を文章で書いて畳む。
