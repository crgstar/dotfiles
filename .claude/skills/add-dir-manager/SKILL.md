...
description: |
  ...
  見えるようにして」「infra-* / management-* / apps-* に一律で add-dir」「X リポに
  /path/to/Y を追加」「Z リポから infra-private を外して」「グループ一括セットアップ」
...
| 「X に Y を追加」「全 infra-* に Z を足して」 | `add` |
| 「X から Y を外して」「Z を削除」 | `remove` |
| 「infra-* グループを一括クロスリンク」「sibling を互いに見えるように」 | `group-setup` |
...
```bash
ls -d /Users/alice/projects/infra-*
```
...
> - infra-dev, infra-local, infra-management-prd, ...
...
```bash
SKILL=/Users/alice/.claude/skills/add-dir-manager
bash $SKILL/scripts/addir.sh group-setup \
  /Users/alice/projects/infra-dev \
  /Users/alice/projects/infra-local \
  ...
```
...
ユーザ「infra-* の add-dir 状況を教えて」

```bash
# Step 2
REPOS=$(ls -d /Users/alice/projects/infra-*)
# Step 4
...
ユーザ「infra-dev に /Users/alice/projects/apps-foo を追加して」

```bash
bash ~/.claude/skills/add-dir-manager/scripts/addir.sh add \
  /Users/alice/projects/infra-dev \
  /Users/alice/projects/apps-foo
```
...
ユーザ「全 infra-* に /Users/alice/projects/apps-foo を足して」

確認 → 各リポについてループで `add` を呼ぶ:

```bash
for repo in /Users/alice/projects/infra-*; do
  bash ~/.claude/skills/add-dir-manager/scripts/addir.sh add \
    "$repo" /Users/alice/projects/apps-foo
done
```

### 例 4: remove

ユーザ「infra-prd から infra-private を外して」

```bash
bash ~/.claude/skills/add-dir-manager/scripts/addir.sh remove \
  /Users/alice/projects/infra-prd \
  /Users/alice/projects/infra-private
```

### 例 5: group-setup

ユーザ「apps-* グループを一括クロスリンクして」

```bash
MEMBERS=$(ls -d /Users/alice/projects/apps-*)
# 確認 → 実行
