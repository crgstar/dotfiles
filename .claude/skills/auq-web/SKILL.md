...
選択肢を出す前に「なぜ今この選択か」を desc 冒頭に 1〜2 文で添える。前提を共有しないまま問うとループになる。
...
HTML は **ファイル経由を既定**にする。heredoc 経由は HTML 内に shell が解釈しうる文字
(バッククォート / `$` / 連続 quote / heredoc 終端と被るトークン) が混ざったときの事故が
最多のため (`Write` ツールならこの懸念がない)。

```
Write ツールで /tmp/auq-web-current.html に HTML を書く
```

固定 path を使う理由: port 7777 が固定で並行起動を許していないので、tmp ファイルも並行衝突しない。
...
| port 7777 衝突メッセージ | port は固定でセッション間で共有されるため、まず占有プロセスの `--input` を確認して所有者を見分ける。自分が起動した入力ファイルなら前のタブが残っているだけなので「画面を閉じてから再実行してください」とユーザに伝える。別の作業ディレクトリを入力とする auq-web が掴んでいる場合は、他セッションの待機中の回答を壊すため**勝手に停止せず、停止してよいかユーザに確認する** |
