...
ユーザと合意済みの決定には「合意済み」と明記し、著者の判断・推測と区別する。受け手が確認すべき箇所を後者だけに絞れる。前 baton から引き継いだ推測項目は、その処遇（合意済み・継続未解決・今回スコープ外で削除）を併記する。
...
Write 実行直前に本文全体を走査し、`/tmp/` または `$TMPDIR` を含む行が残っていないか最終確認する。
