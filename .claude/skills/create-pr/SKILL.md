...
| セクション | 必須 | いつ入れるか | 中身 |
|----------|----|------------|------|
| 変更内容 / Summary | ◯ | 常に | 何を変えたかを箇条書きで。変更の中身に限定し「別コミット」「先行コミット」等の実装組織情報は書かない（レビュワはコミット一覧から読める） |
...
「方針」候補は思いつく限り網羅的に列挙してユーザに提示し、承認を得た案だけ本文に残す。本文の圧縮・トリム・差し替えでも同じく適用する（先回りで構成を変えることは、本文の意図をユーザに代わって決めることになる）。
