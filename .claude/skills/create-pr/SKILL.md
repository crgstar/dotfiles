...
「方針」は「diff だけ見たレビュアーが何を読み違えるか」だけを問いとして書く。diff から自明な実装詳細や一般的な背景を混ぜると、レビュアーが本当に読み違えるポイントが埋もれる。
...
PR 本文はセッション外のレビュアーが読む文書。提示前に作業ログ由来の識別子（フェーズ記号・finding 番号など）や未指示のセクション（「その他」・運用メモ等）を除く。
