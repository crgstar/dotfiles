...
3. **ドラフトを見せて確認を取る**: 本文は「本文の分量」に従う。PRコメントは他人に見える公開行為なので、投稿先(ファイル:行、対象コミット)と本文を提示し、ユーザーの了承が出るまで `gh api` は叩かない。
...
短縮は別コンテキスト(サブエージェント)に任せ、基準と段落ごとの判定例(「この段落は要る」「これは調べれば分かる」)を渡す。自分が書いた文は自分では削りきれないため。
