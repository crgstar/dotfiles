...
## Claude Code の hook permission 仕様（検証済み）

- PreToolUse hook の `permissionDecision` は permission を「厳しくする方向」にしか効かない
- `deny` / `ask` は静的ルールに勝つが、`allow` は静的 `ask` を上書きできない
- よって「デフォ allow + 危険パターンだけ hook で ask/deny 格上げ」が正解パターン
